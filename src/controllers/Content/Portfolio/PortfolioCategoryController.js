// controllers/Content/Portfolio/PortfolioCategoryController.js

const { asyncHandler } = require("../../../middlewares");
const { PortfolioCategory } = require("../../../models");
const {
  sendResponse,
  ObjectIdChecker,
  CustomError,
...
const getAllCategories = async (req, res) => {
  ...
  const categories = await PortfolioCategory.getAllCategories();
...
const getOneCategory = async (req, res) => {
  ...
  //perform query on database
  const category = await PortfolioCategory.getOneCategory(categoryId);
...
const createOneCategory = async (req, res) => {
  ...
  //perform query on database
  const portfolioCategory = await PortfolioCategory.createOneCategory(
    updatedData
  );
  return sendResponse(
    res,
...
const updateOneCategory = async (req, res) => {
  ...
  //perform query on database
  const updatedCategory = await PortfolioCategory.updateOneCategory({
    categoryId,
    updatedData,
  });
  return sendResponse(
...
const deleteOneCategory = async (req, res) => {
  ...
  //perform query on database
  const deletedCategory = await PortfolioCategory.deleteOneCategory(categoryId);
  return sendResponse(
    res,
    200,
